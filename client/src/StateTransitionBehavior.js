...
function folderItemSize(state, folderId) {
  ...
  if (folder) {
    const itemChildrenLength = folder.itemChildren
      ? folder.itemChildren.length
      : 0;
    const folderChildrenLength = folder.folderChildren
...
export const handleTaskflowChange = async (state, taskflow) => {
  ...
  const actions = [];
  try {
    jobs = Object.keys(taskflow.jobMapById).map(
      (id) => taskflow.jobMapById[id]
    );
    tasks = Object.keys(taskflow.taskMapById).map(
  ...
  if (
    (jobs.length && jobs.every((job) => job.status === 'terminated')) ||
    ...
    !allComplete &&
    jobs.length + tasks.length > 0 &&
    !jobs.some((job) => job.status === 'terminating')
  ) {
  ...
  if (taskflow.simulation && state.simulations.mapById[taskflow.simulation]) {
    ...
    if (
      (simulationStatus.length === 2 &&
        simulationStatus[0] !== simulationStatus[1]) ||
      simulationsStatus[simulation._id] !== simulationStatus[1]
    ) {
      const metadata = Object.assign({}, simulation.metadata, {
  ...
  if (taskflow.flow.meta) {
    ...
    if (
      !tfCluster &&
      tfClusterId.length &&
      state.auth.user._id === get(tfCluster, 'userId')
    ) {
      ...
      if (taskflow.flow.meta.cluster && taskflow.simulation) {
        ...
        if (
          tfSimulation &&
          tfCluster.config &&
          !tfCluster.config.simulation &&
          state.auth.user._id === tfCluster.userId
        ) {
          const simulation = {
  ...
  if (
    ...
    if (
      allComplete &&
      taskflow.simulation &&
      state.simulations.mapById[taskflow.simulation]
    ) {
      const sim = state.simulations.mapById[taskflow.simulation];
